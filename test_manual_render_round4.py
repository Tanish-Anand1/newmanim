...
class SumOfOdds(Scene):
    def construct(self):
        ...
        title = Text("Sum of First n Odd Integers = n²", font_size=36, color="#e0e0e3")
        
        # Apply strict frame scaling logic
        frame_width = config.frame_width * 0.9
        if title.width > frame_width:
            title.scale(frame_width / title.width)
        ...
        eq2.next_to(title, DOWN, buff=0.5)

        self.play(FadeIn(dots2), ReplacementTransform(eq1, eq2))
        self.wait(1)
        ...
        eq3.next_to(title, DOWN, buff=0.5)

        self.play(FadeIn(dots3), ReplacementTransform(eq2, eq3))
        self.wait(1)
        ...
        eq4.next_to(title, DOWN, buff=0.5)

        self.play(FadeIn(dots4), ReplacementTransform(eq3, eq4))
        self.wait(2)

        # Step 5: Generalize
        eq5 = MathTex("1 + 3 + 5 + ... + (2n-1) = n^2", font_size=40, color="#e0e0e3")
        
        # Scale all equations to fit portrait bounds
        for eq in [eq1, eq2, eq3, eq4, eq5]:
            if eq.width > frame_width:
                eq.scale(frame_width / eq.width)
                
        eq5.next_to(title, DOWN, buff=0.5)
